...
def display_splash_screen():
    ...
    start_pos = start_text.get_rect(center=(window_width/2, window_height*3/4))
...
def generate_poem():
    ...
    for i, line in enumerate(lines):
        ...
        for j in range(len(words)):
            ...
            line_rect = line_surface.get_rect(topleft=(200, window_height/2 - 180 + (i * 60)))
            window.blit(line_surface, line_rect)
            pygame.display.flip()
    ...
    if gamestate == GAME_STATE_RUNLEVEL and sum(deercaught) + sum(deerlost) == 15:
        ...
        if sum(deercaught) >= 8:
            if nexustype == NEXUS_TYPE_PMEC:
                ...
                summary_pos = summary_text.get_rect(center=(window_width / 2, window_height * 3 / 4 + 120))
        ...
        enter_pos = enter_text.get_rect(center=(window_width/2, window_height*3/4 + 180))
        window.blit(enter_text, enter_pos)
